...
from .bantu import append_to_file
...
# decoded_string = [remove_color_codes(s) for s in color_string]
# Function to decode strings containing color escape sequences
def decode_color_strings(color_strings):
    decoded_strings = [remove_color_codes(s) for s in color_strings]
...
class Widget:
    ...
    __slots__ = ('_mutate', '_handle', '_visual', '_delegate', '_validate', '_escapable', '_product', '_callback')
    ...
    def invoke(self,
        ...
        # coba kita bypass ini, krn mau bikin custom callback
        # handle soalnya kasih argumen gak penting ke callback
        # ternyata dipake utk pindahkan cursor
        ...
        # self._escapable = escapable
        nilai = self._visual.get()
        pertama, kedua = nilai  # pertama adlh list of list
        index, panjang = kedua

        # terbuka = ['👉', ' ', '📁', ' ', '.', 'c', 'i', 'r', 'c', 'l', 'e', 'c', 'i']
        # terbuka = ['👉', ' ', '📄', ' ', '.', 'g', 'i', 't', 'i', 'g', 'n', 'o', 'r', 'e']
        # terbuka = ['👉', ' ', 'Q', 'u', 'i', 't']
        terbuka = decode_color_strings(pertama[index][:panjang])
        terbuka = ''.join(terbuka)
        # append_to_file('surveywidget.txt', f"""Widget/invoke
        # type visual get = {type(nilai)}
        # panjang nilai = {len(nilai)}
        # self._visual.get = index {kedua[0]} adlh visual {pertama[index]}, panjang = {kedua[1]}
        # terbuka = {terbuka}
        # info = {info}\n""")
        if self._callback:
            self._callback(terbuka)
...
def _start(multi_pre : _type_start_multi_pre,
    ...
    # append_to_file('surveywidget.txt', f"""_start
    # info_get = {info_get()}
...
class BaseMesh(Widget, controls = _BaseMesh_controls):
    ...
    def __init__(self,
        ...
        # append_to_file('surveymesh.txt', f"""
        # tiles={tiles}
        # visual={visual}
        # focus={self._focus}
        # """)
...
class BaseList(BaseMesh, controls = _BaseList_controls):
    ...
    def __init__(self,
                ...
                #  view_max   : _type_BaseList_init_view_max    = 7,
                 view_max   : _type_BaseList_init_view_max    = 10,
                #  focus_color: _type_BaseList_init_focus_color = _colors.basic('cyan'),
                 focus_color: _type_BaseList_init_focus_color = _colors.basic('green'),
                #  focus_mark : _type_BaseList_init_focus_mark  = '> ',
                 focus_mark : _type_BaseList_init_focus_mark  = '➡️ ',
...
class Select(BaseList):
    ...
    def __init__(self,
        ...
        def get_tile(value):
            ...
            # get_tile, value=.. (Up one level), option=<vendor.survey.survey._widgets.Input object at 0x00000211E67A7220>
            # ...
            # get_tile, value=📄 survey.txt, option=<vendor.survey.survey._widgets.Input object at 0x0000017B6FE0AEC0>
            # get_tile, value=📄 surveyor.txt, option=<vendor.survey.survey._widgets.Input object at 0x0000017B6FE0AF20>

            # lines = _helpers.split_lines(value)
            # index = len(value)
            # y, x = _helpers.text_index_to_point(lines, index)

            # append_to_file('survey.txt', f"""get_tile, value={value},
            # option={str(option)}
            # value = {value}
            # index = {index}
            # lines = {lines}
