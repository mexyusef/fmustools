...
class Repl(object):
    ...
    def write2file(self):
        ...
        if os.path.exists(fn):
            mode = self.interact.file_prompt('%s already exists. Do you '
                                               'want to (c)ancel, '
                                               ' (o)verwrite or '
                                               '(a)ppend? ' % (fn, ))
            if mode in ('o', 'overwrite', 'overwrite'):
                mode = 'w'
            elif mode in ('a', 'append', 'append'):
                mode = 'a'
            else:
                self.interact.notify('Save cancelled.')
                return
        ...
        try:
            ...
            self.interact.notify("Error writing file '%s': %s" % (fn, e))
        else:
    ...
    def copy2clipboard(self):
        ...
        if self.clipboard is None:
            self.interact.notify('No clipboard available.')
            return
        ...
        try:
            ...
            self.interact.notify('Could not copy to clipboard.')
        else:
            self.interact.notify('Copied content to clipboard.')
    ...
    def do_pastebin(self, s):
        ...
        if s == self.prev_pastebin_content:
            self.interact.notify('Duplicate pastebin. Previous URL: %s. '
                                   'Removal URL: %s' %
                                 (self.prev_pastebin_url,
        ...
        if removal_url is not None:
            self.interact.notify('Pastebin URL: %s - Removal URL: %s' %
    ...
    def prompt_undo(self):
        ...
        n = self.interact.file_prompt(
            "Undo how many lines? (Undo will take up to ~%.1f seconds) [1]"
            % (est,))
        try:
            if n == '':
                n = '1'
            ...
            self.interact.notify('Undo canceled', .1)
            return 0
        else:
            if n == 0:
                self.interact.notify('Undo canceled', .1)
                ...
                #                    n)
                self.interact.notify('Undoing %d line... (est. %.1f seconds)' % (n, est), .1)
            return n
    ...
    def edit_config(self):
        if not os.path.isfile(self.config.config_path):
            if self.interact.confirm("Config file does not exist - create "
                ...
                try:
                    ...
                    self.interact.notify("Error writing file '%s': %s" %
        ...
        try:
            if self.open_in_external_editor(self.config.config_path):
                self.interact.notify('bpython config file edited. Restart '
                                       'bpython for changes to take effect.')
        except OSError as e:
            self.interact.notify('Error editing config file: %s' % e)
