...
class DirectoryTree(TreeControl[DirEntry]):
    ...
    async def load_directory(self, node: TreeNode[DirEntry]):
        ...
        # input(f"path {path}... ")
        try:
            scandir_sebelum_listify = scandir(path)
            dirlist = list(scandir_sebelum_listify)
            directory = sorted(
                dirlist, key=lambda entry: (not entry.is_dir(), entry.name)
            )
            for entry in directory:
                # .add() terima label dan data
                # tiap node kita beri data = DirEntry
                await node.add(entry.name, DirEntry(entry.path, entry.is_dir()))
            node.loaded = True
            await node.expand()
            self.refresh(layout=True)
        except Exception as err:
            print(f'''[vendor.textual.widgets._directory_tree]
            gagal: {err}
            ''')
    ...
    async def handle_tree_click(self, message: TreeClick[DirEntry]) -> None:
        ...
        if not dir_entry.is_dir:
            ...
            await self.emit(DirClick(self, dir_entry.path))
