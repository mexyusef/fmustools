...
def generate_createform(configuration):
  """
  ...
  di sini kita iterate lewat configuration['models'] yg hanya berisi label+type
  """
  # form __TEMPLATE_FORM_ITEM_LIST
  tables = configuration['table_nodes']
  for index, tbl in enumerate(tables,1):
    form_items = []
    # for model_item in configuration['models'][tbl.model]:
...
def generate_formprovider(configuration):
  tables = configuration['table_nodes']
  for index, tbl in enumerate(tables,1):
...
def generate_formwrapper(configuration):
  tables = configuration['table_nodes']
  for index, tbl in enumerate(tables,1):
...
def generate_formlist(configuration):
  tables = configuration['table_nodes']
  for index, tbl in enumerate(tables,1):
...
def generate_formmodal(configuration):
  tables = configuration['table_nodes']
  for index, tbl in enumerate(tables,1):
    content = form_modal
...
def generate_updateform(configuration):
  tables = configuration['table_nodes']
  for index, tbl in enumerate(tables,1):
    form_items = []
    for model_item in configuration['models'][tbl.model]:
...
def generate_the_model(configuration):
  tables = configuration['table_nodes']
  for index, tbl in enumerate(tables,1):
...
def generate_react_component(configuration):
  generate_createform(configuration)
  generate_formprovider(configuration)
  generate_formwrapper(configuration)
  generate_formlist(configuration)
  generate_formmodal(configuration)
  generate_updateform(configuration)
  generate_the_model(configuration)


def generate_menu_json(configuration):
  tables = configuration['table_nodes']
  menu_items = []
  for index, tbl in enumerate(tables,1):
...
def generate_app_routes(configuration):
  tables = configuration['table_nodes']
  import_parts = []
  route_parts = []
  for index, tbl in enumerate(tables,1):
