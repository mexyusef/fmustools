...
if __name__ == '__main__' or __name__ == 'winless2': # jk sama dg nama file = standalone
...
QMenu::item:selected {
    border-radius: 2px;
    background-color: rgba(85, 232, 32, 232);
}
...
def resize_screen_ratio(object, screenw, screenh, posx_ratio=1/2, ratio=1/6):
    ...
    object.move(QPoint(posx, posy)) # utk widget kita
...
class MyWindow(QtWidgets.QMainWindow):
    ...
    def move(self, pos):
        if self.windowState() == Qt.WindowMaximized or self.windowState() == Qt.WindowFullScreen:
            return
        super(MyWindow, self).move(pos)

    def showMaximized(self):
        super(MyWindow, self).showMaximized()
        self.layout().setContentsMargins(0, 0, 0, 0)

    def showNormal(self):
        super(MyWindow, self).showNormal()
    ...
    def _resizeWidget(self, pos):
        ...
        print(f"""self.setGeometry(x, y, w, h)
    ...
    def initUI(self):
        ...
        self.setCentralWidget(fakeWidget)
    ...
    def setTrayMenu(self):
        show_action = QAction("Menu Show", self)
        quit_action = QAction("Menu Exit", self)
        hide_action = QAction("Menu Hide", self)
...
class MyInternalWindow(QWidget):

    def __init__(self, *args, **kwargs):
        super(MyInternalWindow, self).__init__(*args, **kwargs)
        layout = QVBoxLayout(self, spacing=0)
        layout.setContentsMargins(0, 0, 0, 0)
        self.tab = QTabWidget()
...
def main():
    ...
    def unregister():
        ...
        keybinder.register_hotkey(window.winId(), "Shift+Ctrl+A", callback_hide)
    ...
    # screenw, screenh = screen_geometry.width(), screen_geometry.height()
    # resize_screen_ratio(window, screenw, screenh, 1/2, 1/2)

    # window.setStyleSheet(frameless_stylesheet + contextmenu_stylesheet)
    window.setStyleSheet(contextmenu_stylesheet)
    window.setWindowTitle(f'ctrl shift a/e')

    # internal = MyInternalWindow(window)
    # window.setWidget(internal)
