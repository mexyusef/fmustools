...
class BerbagaiWidget(QWidget):
    ...
    def initUI(self):
        ...
        self.tab_berbagai = MKHelpWidget(parent=self, rootpath=joiner(sidoarjodir, 'data/karya'), copy_after_insert=True, extra_buttons=[kenza_folder_button, explore_kenza_folder_button])
        self.main_tab.addTab(self.tab_berbagai, 'Berbagai')
        ...
        self.tab_berbagai = MKHelpWidget(parent=self, rootpath=joiner(sidoarjodir, 'database/geura/memorizer'), copy_after_insert=True, extra_buttons=[])
        self.main_tab.addTab(self.tab_berbagai, 'Memo')

        self.tab_mkhelp = MKHelpWidget(parent=self, rootpath=mkhelp_initial_path)
        self.main_tab.addTab(self.tab_mkhelp, 'Help MK2')

        # self.tab_001 = MKHelpWidget(parent=self, rootpath=joiner(QUICKS_DIR, 'devops'))
        # self.main_tab.addTab(self.tab_001, 'DevOp')
