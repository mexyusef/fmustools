...
class Application(Frame):
    ...
    def open_file(self, event=None, file_name=None):
        ...
        if os.path.isfile(file_name):
            with open(file_name, 'r', encoding='utf8') as fd:
                self.text_box.insert('0.0', fd.read())
    ...
    def about_prog(self, event=None):
        ...
        Label(w, text="Simple File Viewer and Editor").grid(column=1, row=3, **options)
...
def tkeditor(filename=None):
    Application.main(filename=filename)

if __name__ == '__main__':
    tkeditor(filename=None)
