...
class Window(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        layout.addWidget(QPushButton('Python calls functions in qml',
    ...
    def onTimeout(self):
        # 定时器发送信号通知qml
        self.timerSignal.emit('sent by timer:' + str(time()))

    def callQmlFunc(self):
        # 主动调用qml中的appendText函数
        engine.rootObjects()[0].appendText('I was called by Python')

    @pyqtSlot(int)
    def onValueChanged(self, value):
        # qml中的自定义信号valueChanged所绑定的槽函数
        self.resultView.append('pull bar value: %s' % value)

    @pyqtSlot(str, result=str)  # 可以获取返回值
    def testSlot(self, name):
        # 被qml调用的函数
        self.resultView.append('I was actively called: %s' % name)
        return str(len(name))
...
if __name__ == '__main__':
    ...
    engine.objectCreated.connect(
        lambda obj, _: QMessageBox.critical(None, 'error', 'Failed to run, please check') if not obj else 0)
