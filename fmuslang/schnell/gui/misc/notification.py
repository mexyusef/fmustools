...
class NotificationWindow(QListWidget):
    ...
    def error(cls, title, message, callback=None):
        ...
        item.setSizeHint(QSize(width, 300))
        cls._instance.setItemWidget(item, w)
...
if __name__ == '__main__':
    ...
    layout.addWidget(QPushButton('Info', w, clicked=lambda: NotificationWindow.info('<i>judul info</i>', 'This is a message that will automatically close', callback=callback)))
    
    layout.addWidget(QPushButton(
        'Success', w, clicked=lambda: NotificationWindow.success('judul sukses', 'This is a message that will automatically close', callback=callback)))
    layout.addWidget(QPushButton(
        'Warning', w, clicked=lambda: NotificationWindow.warning(
            'judul peringatan',
            'This is prompt copy This is prompt copy This is prompt copy This is prompt copy.',
            callback=callback)))
    layout.addWidget(QPushButton(
        'Error', w, clicked=lambda: NotificationWindow.error(
            'judul gagal',
            '<html><head/><body><p><span style=" font-style:italic; color:teal;">This is the prompt copy This is the prompt copy This is the prompt copy This is the prompt copy This is the prompt copy This is the prompt copy This is the prompt copy This is the prompt copy</span></p></body></html>',
            callback=callback)))
    w.show()

    #     NotificationIcon.init()
    #     ww = NotificationItem('hint', '<html><head/><body><p><span style=" font-style:italic; color:teal;">这是提示文案这是提示文案这是提示文案这是提示文案这是提示文案这是提示文案这是提示文案这是提示文案</span></p></body></html>', None,
