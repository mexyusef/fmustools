...
def kolom_as_params(table, exclude=''):
	...
	if exclude:
		columns = [f'{col.label}: {col.type}' for col in table.children if col.label != exclude]
	return ', '.join(columns)
...
def gen_template_db_init(RootNode, return_backend=False, print_info=False, use_real_tab=False):
	dbvalues = {}
	dblines = []
	dbinfo = RootNode
	...
	db_backend = 'sqlite'
	if 'host' in dbvalues or 'port' in dbvalues or 'username' in dbvalues or 'password' in dbvalues:
		db_backend = 'postgres'
