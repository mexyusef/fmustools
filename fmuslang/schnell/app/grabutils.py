...
class Grabber(tk.Tk):
	...
	def on_button_press(self, event):
		...
		self.rect = self.canvas.create_rectangle(self.x, self.y, 2, 2, outline='green')
	...
	def on_button_release(self, event):
		...
		if self.callback:
			print('[grabutils][Grabber] calling callback with bbox:', bbox)
			self.callback(bbox)
...
def grabber(callback):
	"""
	callback harus menerima bbox
	"""
	Grabber(callback).mainloop()
