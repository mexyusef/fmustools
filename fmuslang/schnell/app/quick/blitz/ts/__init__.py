...
class Coordinator:
	...
	def __init__(self, RootNode, filename, project_dir='__INPUT__'):
		...
		if dirname(self.mkfile_input) != disini:
			'''
			<disini>/js/node/choose.fmus
			<disini>/output.fmus
			'''
			# indah4(f'''
			# disini = {disini}
			# self.mkfile_input = {self.mkfile_input}
			# lokasi input = {dirname(self.mkfile_input)}
			# self.mkfile_output = {self.mkfile_output}
			# ''', warna='red')
			# input(' press... ')
	...
	def save_file(self):
		...
		print('content input adlh:', self.mkfile_input_content)
