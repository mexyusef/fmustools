...
url = 'https://raw.githubusercontent.com/dexplo/bar_chart_race/master/data/urban_pop.csv'
df0 = pd.read_csv(url)
df = df0.melt(id_vars=['year'], var_name='country', value_name='population') # .to_csv('negara.csv')
pilihan_warna = ["#adb0ff", "#ffb3ff", "#90d595", "#e48381", "#aafbff", "#f7bb5f", "#eafb50"]
list_countries = list(df["country"].unique())

tahun_min = df['year'].min()
tahun_max = df['year'].max()
dff_dx_dict = {}
for tahun in range(tahun_min, tahun_max+1):
    hasil_dff = df[df['year'].eq(tahun)].sort_values(by='population', ascending=True).tail(10)
    hasil_dx = hasil_dff['population'].max() // 200
    # hasil_warna = [colors[group_lk[x]] for x in hasil_dff['country']]
    hasil_warna = [random.choice(pilihan_warna) for x in hasil_dff['country']]
    value_name = zip(hasil_dff['population'], hasil_dff['country'])
    dff_dx_dict[tahun] = {
        'dff': hasil_dff,
        'dx': hasil_dx,
        'color': hasil_warna,
...
class MatplotRace(QWidget):
    ...
    def draw_barchart(self, current_year):
        ...
        self.static_canvas_axes.barh(dff['country'], dff['population'], color=warna)
    ...
    def sekali(self):
        ...
        self.static_canvas_axes.text(0, 1.15, 'Judul apa saja', transform=self.static_canvas_axes.transAxes, size=24, weight=600, ha='left', va='top')
