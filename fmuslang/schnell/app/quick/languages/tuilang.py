...
from schnell.app.transpiler.frontend.main import process_language
from schnell.app.stringutils import tabify_content_space, tabify_contentlist_space
...
from vendor.textual.app import App
from vendor.textual.reactive import Reactive
from vendor.textual.widgets import (
    Header, Footer, 
    FileClick, DirClick, 
    ScrollView, Static, Placeholder,
    DirectoryTree,
)
from vendor.textual.widget import Widget
from vendor.textual.views import GridView
        ...
        if jenis == 'element_name':
            ...
            for tupleitem in anak(item):
                ...
                if jenis2 == 'item_key_value':
                    ...
                    print(f'  attr of {namaparent}/{itemid}, row={parentid} k=v => {k}={v}')
                    if namaparent == 'col':
...
file_write('jalan.py', kode_output)
print(kode_output)
