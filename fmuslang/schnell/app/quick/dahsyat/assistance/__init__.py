...
from app.stringutils import tab_space4, tab_tab, tab_space2
...
class Coordinator:
	...
	def app_content(self):
		from app.libpohon.app_content import app_content
		template_node_app_content = app_content(self.tables, self.node_app_content, tab=tab_space2)
	...
	def generate_config(self):
		...
		for index, tbl in enumerate(self.tables,1):
			...
			per_table = app_content_model(tablename, self.file_path_config_content, tab=tab_space2)
	...
	def generate_store(self):
		...
		store_imports = app_content(self.tables, self.file_store_import_content, tab=tab_space2)
		store_reducers = app_content(self.tables, self.file_store_reducer_content, tab=tab_space2)
	...
	def generate_appcontent(self):
		...
		appcontentlines = app_content(self.tables, self.file_appcontent_content, tab=tab_space2)
		...
		for index, tbl in enumerate(self.tables,1):
			tablename = tbl.model
			tablename_lower = tbl.model.lower()
			actions_content = app_content_model(tablename, self.file_actions_content, tab=tab_space2)
			...
			constants_content = app_content_model(tablename, self.file_constants_content, tab=tab_space2)
			...
			reducers_content = app_content_model(tablename, self.file_reducers_content, tab=tab_space2)
