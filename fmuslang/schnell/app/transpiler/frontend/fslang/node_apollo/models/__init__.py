...
def generate_imports(tables, mkfile_input_content):
	...
	for index, tbl in enumerate(tables,1):
		...
		apps_typedefs_bycomma.append(tablename_lower + 'Type')
		apps_resolvers_bycomma.append(tablename_lower + 'Resolver')

	# core/connection/graphql/index.js
	# apps_imports_byline = []
	mkfile_input_content = mkfile_input_content.replace('__TEMPLATE_APPS_IMPORTS__', '\n'.join(apps_imports_byline))
	# apps_typedefs_bycomma = []
	mkfile_input_content = mkfile_input_content.replace('__TEMPLATE_APPS_TYPEDEFS_LIST__', ', '.join(apps_typedefs_bycomma))
	# apps_resolvers_bycomma = []
	mkfile_input_content = mkfile_input_content.replace('__TEMPLATE_APPS_RESOLVERS_LIST__', ', '.join(apps_resolvers_bycomma))
	return mkfile_input_content
...
def gen_models(tables, mkfile_input_content):
	...
	mkfile_input_content = generate_imports(tables, mkfile_input_content)
