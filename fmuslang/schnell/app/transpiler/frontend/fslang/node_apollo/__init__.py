...
class Coordinator:
	...
	def __init__(self, RootNode, project_dir='__INPUT__'):
		...
		self.mkfile_output = joiner(get_cwd(), 'node_apollo_backend_output.mk')
		...
		self.node_model = file_content(joiner(disini, 'templates/nodeapollo_app_content_v1.tpl'))
		self.hasil_columnify = columnify(self.tables, 'node_apollo')
	...
	def app_content(self):
		...
		tpl_appcontent = file_content(joiner(disini, 'templates/nodeapollo_app_content_v1.tpl'))
		# app_content terima "content" dari file tpl (file_content), bukan namafilenya (joiner)
		template_node_app_content = app_content(self.tables, tpl_appcontent)
		print(f'''
		node_apollo app_content
		isi mk entry:
		[{template_node_app_content}]
		self.tables = {self.tables}
		tpl_appcontent = {tpl_appcontent}
		untuk ganti: __TEMPLATE_SERVER_APP_CONTENT
		''')
		# input('Press utk teruskan ... ')
	...
	def app_model(self):
		"""
		ternyata ini gak dipanggil, tapi .models/__init__
		backend: model/sequelize
		"""
		from app.libpohon.app_content import app_content
		template_node_app_content = app_content(self.tables, self.node_model)
		# self.mkfile_input_content = self.mkfile_input_content.replace('__TEMPLATE_SERVER_APP_CONTENT', template_node_app_content)
		per_app_import_tpl = "import { __TABLENAME_LOWER__Type, __TABLENAME_LOWER__Resolver } from 'A/__TABLENAME_LOWER__/__TABLENAME_LOWER__.graphql';"
		apps_imports_byline = []
		apps_typedefs_bycomma = []
		apps_resolvers_bycomma = []

		for meja in self.tables:
			tablename = meja.model
			tablename_lower = tablename.lower()
			apps_imports_byline.append(per_app_import_tpl.replace('__TABLENAME_LOWER__', tablename_lower))
			apps_typedefs_bycomma.append(tablename_lower)
			apps_resolvers_bycomma.append(tablename_lower)
			generated_fields = columnify_and_transform(self.tables, 'node_apollo', tablename)
			per_table = template_node_app_content.replace('__FIELDS__', generated_fields)
			per_table = per_table.replace('__TABLENAME__', tablename)
			per_table = per_table.replace('__TABLENAME_LOWER__', tablename_lower)
			per_table = per_table.replace('__TABLENAME_UPPER__', tablename.upper())
			per_table = per_table.replace('__TABLENAME_LOWER_PLURAL__', tablename_lower + 's')
			per_table = per_table.replace('__TABLENAME_UPPER_PLURAL__', tablename.upper() + 's')
			per_table = per_table.replace('__TABLENAME_CAP_PLURAL__', tablename.capitalize() + 's')
			per_table = per_table.replace('__TAB(1)', tab(1))
		...
		# apps_imports_byline = []
		self.mkfile_input_content = self.mkfile_input_content.replace('__TEMPLATE_APPS_IMPORTS__', '\n'.join(apps_imports_byline))
		# apps_typedefs_bycomma = []
		self.mkfile_input_content = self.mkfile_input_content.replace('__TEMPLATE_APPS_TYPEDEFS_LIST__', ', '.join(apps_typedefs_bycomma))
		# apps_resolvers_bycomma = []
		self.mkfile_input_content = self.mkfile_input_content.replace('__TEMPLATE_APPS_RESOLVERS_LIST__', ', '.join(apps_resolvers_bycomma))
	...
	def model(self):
		...
		self.mkfile_input_content = gen_models(self.tables, self.mkfile_input_content)
	...
	def generate(self):
		...
		self.model()
		self.save_file()
		print('selesai generate node_apollo')
