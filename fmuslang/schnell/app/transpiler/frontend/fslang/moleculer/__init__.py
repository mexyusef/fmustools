...
class Coordinator:
  ...
  def __init__(self, RootNode, project_dir='__INPUT__'):
    ...
    self.hasil_columnify = columnify(self.tables, 'moleculer')
  ...
  def app_model(self):
    ...
    for meja in self.tables:
      ...
      generated_fields = columnify_and_transform(self.tables, 'moleculer', tablename)
  ...
  def generate(self):
    """
    sementara moleculer belum kita isi
    """
    self.mkfile()
    # self.model()
    self.save_file()
    print('selesai generate moleculer')
