...
from app.stringutils import left_right_joinify_contentlist, tabify_content, tabify_contentlist
disini = here(__file__)

tpl_approutes = joiner(disini, '../templates/app_routes_v1.tpl')
tpl_appindex = joiner(disini, '../templates/app_init_v1.tpl')
tpl_appmodel = joiner(disini, '../templates/app_models_v1.tpl')
tpl_appforms = joiner(disini, '../templates/app_forms_v1.tpl')
tpl_appresource = joiner(disini, '../templates/app_resource_v1.tpl')
tpl_appcontent = joiner(disini, '../templates/apps_mk_content_v1.tpl')
...
def generate_app_routes(tables, mkfile_input_content):
	for index, tbl in enumerate(tables,1):
		tablename_lower = tbl.model.lower()
		tablename_case = tbl.model
...
def generate_app_model2(tables, mkfile_input_content):
	from app.libpohon.app_content_model import app_content
	
	for meja in tables:
		tablename = meja.model
		template_appmodel = app_content(tablename, file_content(tpl_appmodel))
		generated_fields = columnify_and_transform(tables, 'flask', tablename)
		
		# generated_fields = f"hasil generated fields: [{generated_fields}]"
		per_table = template_appmodel.replace('__FIELDS__', generated_fields)
		per_table = per_table.replace('__TEMPLATE_COLUMNS__', generated_fields)
		...
		per_table = per_table.replace('__TEMPLATE_TABLENAME_LOWER__', tablename.lower())
		per_table = per_table.replace('__TEMPLATE_TABLENAME_CASE__', tablename)
		...
		column_list_non_id1 = [column.label for column in meja.children]
		column_list_non_id2 = [f"'{item}': self.{item}" for item in column_list_non_id1]

		# ini todict dan tojson sama?

		# __TEMPLATE_TODICT__ = '\n'.join([tab()+item for item in column_list_non_id2])
		__TEMPLATE_TODICT__ = tabify_contentlist(column_list_non_id2, num_tab=3, aslist=True)
		__TEMPLATE_TODICT__ = left_right_joinify_contentlist(__TEMPLATE_TODICT__, ',')
		
		# __TEMPLATE_TOJSON__ = '\n'.join([tab()+item for item in column_list_non_id2])
		__TEMPLATE_TOJSON__ = tabify_contentlist(column_list_non_id2, num_tab=3, aslist=True)
		__TEMPLATE_TOJSON__ = left_right_joinify_contentlist(__TEMPLATE_TOJSON__, ',')

		__COLUMN_LIST__ = ', '.join([f'"{column.label}"' for column in meja.children])
		per_table = per_table.replace('__TEMPLATE_TODICT__', __TEMPLATE_TODICT__)
		per_table = per_table.replace('__TEMPLATE_TOJSON__', __TEMPLATE_TOJSON__)
		per_table = per_table.replace('__COLUMN_LIST__', __COLUMN_LIST__)

		header = f'/myflask2/project/apps/{tablename.lower()}/models.py'
		mkfile_input_content = append_entry_tostring(mkfile_input_content, header, per_table)

	return mkfile_input_content


def generate_app_forms_resource(tables, mkfile_input_content):
	"""
	...
	for index, tbl in enumerate(tables,1):
		...
		mkfile_input_content = append_entry_tostring(mkfile_input_content, header_form, per_table_forms)
		...
		mkfile_input_content = append_entry_tostring(mkfile_input_content, header_resource, per_table_resource)
	return mkfile_input_content
...
def gen_models(tables, mkfile_input_content):
	...
	mkfile_input_content = generate_app_routes(tables, mkfile_input_content)
	# mkfile_input_content = generate_app_model(tables, mkfile_input_content)
	mkfile_input_content = generate_app_model2(tables, mkfile_input_content)
	mkfile_input_content = generate_app_forms_resource(tables, mkfile_input_content)
	return mkfile_input_content
