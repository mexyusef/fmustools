from app.dirutils import joiner, here
...
graphql_types_mapper = type_mapper_by_provider['django_graphene']
...
# from .config import disini
disini = here(__file__)
app_schema_template_filepath = joiner(disini, 'templates/django-graphql-app-schema-v1.tpl')

def value_assignment(tablename, item):
  return f"if {item}:\n{tab(1)}{tablename}.{item} = {item}"


def handle_app_schema(tables, mkfile_input_content):
  ...
  for tblindex, tbl in enumerate(tables,1):
    ...
    tablename = __TEMPLATE_TABLENAME_CASE__
    header = f'/apps/{tablename}/schema.py'
    # body = app_template    
    # start='--%'
    # end='--#'
    # entry_model = f'\n{start} {header}\n' + body + f'\n{end}\n'
    # append_file(output_django_mkfile, entry_model)
    res = append_entry_tostring(mkfile_input_content, header, app_template)
    app_templates_result.append(res)

  return '\n\n'.join(app_templates_result)
...
"""
def handle_main_schema(tables, mkfile_input_content):
  import_separator = '\n'
  ...
  res = mkfile_input_content.replace('__TEMPLATE_SCHEMA_APP_IMPORTS__', importsval)
  res = res.replace('__TEMPLATE_SCHEMA_APP_QUERIES__', queriesval)
  res = res.replace('__TEMPLATE_SCHEMA_APP_MUTATIONS__', mutationsval)
  return res


def generate_graphql(tables, mkfile_input_content):
  res = handle_main_schema(tables, mkfile_input_content)
  res = handle_app_schema(tables, res)
  return res
