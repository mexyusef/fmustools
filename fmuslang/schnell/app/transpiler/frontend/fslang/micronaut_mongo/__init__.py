...
class Coordinator:
  ...
  def __init__(self, RootNode, project_dir='__INPUT__'):
    ...
    self.mkfile_output = joiner(get_cwd(), 'micronaut-mongo-output.mk')
    self.mkfile_input_content = file_content(self.mkfile_input)

    self.tpl_appcontent = file_content(joiner(disini, 'templates/nestjs_app_content_v1.tpl'))

    self.hasil_columnify = columnify(self.tables, 'nest')
  ...
  def app_model(self):
    ...
    for meja in self.tables:
      ...
      generated_fields = columnify_and_transform(self.tables, 'nest', tablename)
