...
class Coordinator:
	...
	def __init__(self, RootNode, project_dir='__INPUT__'):
		...
		self.mkfile_output = joiner(get_cwd(), 'springboot_backend_output.mk')
		self.mkfile_input_content = file_content(self.mkfile_input)

		self.xml_mapper_entry = file_content(joiner(disini, 'templates/xml_mapper_entry.tpl'))
		...
		self.node_app_content = file_content(joiner(disini, 'templates/springboot_app_content_v1.tpl'))
		self.node_model = file_content(joiner(disini, 'templates/springboot_app_rest_model_v1.tpl'))
		self.hasil_columnify = columnify(self.tables, 'springboot')
	...
	def app_xml_mapper(self):
		from .helper import generate_app_xml_mapper
		xml_mapper_content = generate_app_xml_mapper(self.tables)

		from app.libpohon.app_content import app_content
		template_node_app_content = app_content(self.tables, self.xml_mapper_entry)

		# mkfile_template = mkfile_template.replace('__TEMPLATE_APP_XMLMAPPER_ENTRY', xml_mapper_content)
		self.mkfile_input_content = self.mkfile_input_content.replace('__TEMPLATE_APP_XMLMAPPER_ENTRY', xml_mapper_content)
	...
	def mkfile(self):
		...
		self.app_content()
		self.app_xml_mapper()
		from .helper import assign_remaining_variables
		self.mkfile_input_content = assign_remaining_variables(self.tables, self.mkfile_input_content)
	...
	def app_model(self):
		...
		for meja in self.tables:
			...
			generated_fields = columnify_and_transform(self.tables, 'springboot', tablename)
