...
class Coordinator:
  ...
  def __init__(self, RootNode, project_dir='__INPUT__'):
    ...
    self.node_model = file_content(joiner(disini, 'node/mongo_model.tpl'))
    self.hasil_columnify = columnify(self.tables, 'node_antd_mongo')
  ...
  def app_model(self):
    ...
    for meja in self.tables:
      ...
      generated_fields = columnify_and_transform(self.tables, 'node_antd_mongo', tablename)
      ...
      header = f'/np/node-mongodb/src/apps/{tablename.lower()}/models/mongo.js'
  ...
  def app_json(self):
    ...
    for meja in self.tables:
      ...
      # kasih masing-masing kolom dg DQ agar valid json
  ...
  def generate(self):
    ...
    self.model()
    self.save_file()
    print('selesai generate node_antd_mongo')
