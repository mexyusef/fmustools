...
class Coordinator:
  ...
  def __init__(self, RootNode, project_dir='__INPUT__'):
    ...
    self.mkfile_output = joiner(get_cwd(), 'fastapi_backend_output.mk')
    self.mkfile_input_content = file_content(self.mkfile_input)
    self.node_app_content = file_content(joiner(disini, 'templates/fastapi_app_content_v1.tpl'))
    ...
    self.hasil_columnify = columnify(self.tables, 'fastapi')
  ...
  def app_model(self):
    ...
    for meja in self.tables:
      ...
      generated_fields = columnify_and_transform(self.tables, 'fastapi', tablename)
  ...
  def generate(self):
    ...
    self.model()
    self.save_file()
    print('selesai generate fastapi')
