...
def get_tipe_kolom(column, provider='struct_rs'):
	# mapper = type_mapper.get(provider)
...
def struct_rs(tables):
	...
	for tblidx, tbl in enumerate(tables,1):
		...
		for colidx, column in enumerate(tbl.children):
			tipe_kolom = get_tipe_kolom(column)
	...
	# kembali += f'struct_rs => {tablename_case}={tablename_lower} => {fields}'

	kembali += template_struct_instance.replace('__IFNAME', tablename_case).replace('__FIELDS', fields)
	return kembali
