...
controller = joiner(disini, 'templates/help_nest_postgres_controller.tpl')
service = joiner(disini, 'templates/help_nest_postgres_service.tpl')
modelfile = joiner(disini, 'templates/help_nest_postgres_model.tpl')
...
def create_model(tables, tablename_lower, tablename_case):
	...
	# columnify itu list of tables, transform_columns itu beroperate pada satu table	
	provider = 'nest_postgres' # lihat type_mapper_by_provider utk daftar
	...
	hasil3 = tabify_content(hasil_transform)

	provider = 'struct_ts'
	hasil_columnify = columnify(tables, provider)
	print_json(hasil_columnify)
	hasil_transform = transform_columns(tablename_case, tables_with_columns=hasil_columnify, provider=provider)
	hasil6 = tabify_content(hasil_transform)

	modelcontent = modelcontent.replace('__TEMPLATE_MONGOSE_FIELDS__', hasil3)
	modelcontent = modelcontent.replace('__TEMPLATE_TYPESCRIPT_FIELDS__', hasil6)
	return modelcontent
...
def help_nest_postgres(tables):
	...
	fields = ', '.join(columns)
	# kembali += f'help_nest_postgres => {tablename_case}={tablename_lower} => {fields}'
	pemisah = '\n' + '*'*40 + ' __TITLE__' + '\n'*2
