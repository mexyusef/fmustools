...
template_json_definitions = """"definitions": {
	"__TABLENAME_CASE__": {
	}
...
def swagger_json(tables):
	...
	for tblidx, tbl in enumerate(tables,1):
		...
		fields = ', '.join(columns)
		# kembali += f'swagger_json => {tablename_case}={tablename_lower} => {fields}'
		records = (template_json
			.replace('__PATHS__', 
	    		template_json_paths.replace('__TABLENAME_PLURAL_LOWER__', tablename_plural_lower))
			.replace('__DEFINITIONS__', 
	    		template_json_definitions.replace('__TABLENAME_CASE__', tablename_case)
			)
		)
