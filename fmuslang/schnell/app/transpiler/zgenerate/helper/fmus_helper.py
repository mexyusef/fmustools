...
from schnell.app.fileutils import file_content, file_content_add_newline
from schnell.app.transpiler.editor import editor
from schnell.app.greputils import pattern_search_list
from schnell.app.transpiler.zgenerate.helper.redis_helper import daftar
...
def run_fmus(sourcefile, targetfile, title='New FMUS code'):
  ...
  if program.strip():
    ...
    if sourcefile.endswith('.mk'):
      fmus.process_mkfile(targetfile, baris_entry='index/fmus')
...
def fmus_run_file(code, filepath):
  ...
  if code.strip() == '*':
    ...
    if len(hasil) == 1:
      ...
      print('fmus_run_file: ketemu nama file:', hasil, '\n->', sourcefile, '\n=>', filepath)
