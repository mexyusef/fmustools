...
class Generator:
  ...
  def enum_member(self, tree):
    ...
    for item in anak(tree):
      if data(item) == 'nama_identifier':
        nama = token(item).upper() # gak mesti, tapi krn banyak contoh uppercase yo wis
  ...
  def assignment_statement(self, tree, returning=True):
    ...
    for item in anak(tree):
      if data(item) == 'nama_identifier':
        ...
        ass_value, namakelas = self.instantiation_expression(item, returning=True)
      elif data(item) == 'expression_item':
  ...
  def for_of(self, tree):
    ...
    for item in anak(tree):
      if data(item) == 'item_name':
        ...
        itemname = self.nama_jenis_identifier_optional(nama_jenis_identifier_optional)
  ...
  def statement_item(self, contentitem, returning=True):
    ...
    if jenisstmt.data == 'single_statement':
      ...
      jika_mintaresult, namakelas = self.instantiation_expression(jenisstmt, returning=True)
      if not jika_mintaresult.endswith(';'):
        jika_mintaresult += ';'
      if returning:
        return jika_mintaresult
  ...
  def method_content(self, tree):
    ...
    kembali = ''
    # tipe_identifier = ''
    method_modifier, method_name, method_returntype, method_param, method_content = '','','void','',''
    for item in anak(tree):
      if data(item) == 'method_config':
        method_modifier = self.method_config(item)
      elif data(item) == 'function_name':
        nama_identifier_with_typeparams = child1(item)
        method_name = self.nama_identifier_with_typeparams(nama_identifier_with_typeparams)
      elif data(item) == 'tipe_identifier':
    ...
    self.inc()
    kembali = self.tab() + f'{method_modifier} {method_returntype} {method_name}{method_param};'
    self.dec()
    return kembali
