...
from schnell.app.transpiler.zgenerate.refactor.handlers import statement_item
...
def anon_statements(tree, language='py'):
  ...
  for statement in anak(tree):
    if data(statement) == 'statement_item':
      ...
      hasil = statement_item.statement_item(statement, within_statement_list=True, language=language)
