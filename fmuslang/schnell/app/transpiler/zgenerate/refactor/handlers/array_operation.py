...
from schnell.app.transpiler.zgenerate.refactor.handlers import initialize_with_faker
...
from schnell.app.transpiler.zgenerate.refactor.handlers import stringify_anonymous_function_result
from schnell.app.transpiler.zgenerate.refactor.handlers.common import inc, dec, self_tab
from schnell.app.transpiler.zgenerate.refactor.handlers import array_map
...
def array_operation(tree, identifier, language='py'):
  ...
  for item in anak(tree):
    ...
    if data(item) == 'penanda':
      ...
      hasil = nama_identifier_or_literal(nama_identifier_or_literal, language=language)
      ...
      arr?AF#(){}
      '''
      kembali = array_map.array_map(item, language=language) # sementara belum array_filter
      kembali['operation'] = 'filter'
    elif data(item) == 'array_reduce':
      '''
      arr?AR#(){}
      '''
      kembali = array_map.array_map(item, language=language) # sementara belum array_reduce
      ...
      for cucu in anak(item):
        ...
        if jenis == 'allocate':
          ...
          hasil = initialize_with_faker.initialize_with_faker(cucu, language=language)
          kembali += f'{identifier} = {hasil}'
          ...
          mapper_func = anonymous_function.anonymous_function(_anonymous_function, language=language)
          mapper_func = stringify_anonymous_function_result.stringify_anonymous_function_result(mapper_func, language=language)
