...
def class_item(tree, language='py'):
  ...
  for item in anak(tree):
    if data(item) == 'class_config':
      ...
      class_modifier_start, class_modifier_end = class_config.class_config(item, language=language)
      # kembali += prepender + ' '
    elif data(item) == 'class_name':
      # class_name
      if chdata(item) == 'nama_identifier_with_typeparams':
        _nama_identifier_with_typeparams = child(item)
        class_name = nama_identifier_with_typeparams.nama_identifier_with_typeparams(_nama_identifier_with_typeparams, language=language)
      ...
      # class_content
      inc()
      _class_content = class_content.class_content(item, structname=class_name, language=language)
      dec()
