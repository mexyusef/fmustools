...
from schnell.app.stringutils import SQ, DQ, BT
try:
...
class Generator:
  ...
  def expression_item(self, expritem, returning=True):
    ...
    if data(provided_expression) == 'literal':
      ...
      hasil = hasil_in_dict['internal'] if hasil_in_dict['internal'] else hasil_in_dict['external']
  ...
  def string_operation(self, tree, identifier):
    ...
    for item in anak(tree):
      if data(item) == 'penanda':
        ...
        kembali += f'{identifier}.substr({awal}, {result})' # kurang cantik tapi jelas
  ...
  def initialize_with_faker(self, tree):
    ...
    from schnell.app.fakerutils import getfakers
    funcnames = {
      'string': 'word',
      'int': 'pyint',
      'number': 'pyint',
      ...
      if data(item) == 'penanda':
        ...
        for cucu in anak(item):
          ...
          if jenis == 'allocate':
            ...
            tipe_array = f'[]int'
            kembali += f'{identifier} {samadengan} {tipe_array}{{cap: {capacity}}}'
            ...
            mapper_func = self.stringify_anonymous_function_result(mapper_func)
  ...
  def datetime_operation(self, tree, identifier):
    ...
    for item in anak(tree):
      if data(item) == 'datetime_operation_config':
        ...
        if beranak(item):
          ...
          if date_fields:
            ...
            if len(date_fields[2]) == 4:
              year_first = False
            if year_first:
              date_string = f'{date_fields[0]}-{date_fields[1]}-{date_fields[2]}'
            else:
              date_string = f'{date_fields[2]}-{date_fields[1]}-{date_fields[0]}'
        ...
        kembali += f'{identifier}.getMinutes().padStart({identifier}.getMinutes(), 0)'
      elif data(item) == 't_minute':
        kembali += f'{identifier}.getMinutes()'
      elif data(item) == 'ss_second':
        kembali += f'{identifier}.getSeconds().padStart({identifier}.getSeconds(), 0)'
        ...
        from schnell.app.datetimeutils import month3
  ...
  def switch_statement(self, tree, returning=True):
    ...
    for item in anak(tree):
      if data(item) == 'switch_keyword':
        ...
        case_body = self.case_body(item, terbanding, first_case, returning=returning)
  ...
  def anonymous_function(self, tree):
    ...
    for item in anak(tree):
      if data(item) == 'non_arrow_func':
        ...
        if len(badan_fungsi) == 1 and is_arrow_func_or_lambda: # 1 statement -> arrow
    ...
    for item in anak(cucu):
      if data(item) == 'class_config':
        ...
        if chdata(item) == 'nama_identifier_with_typeparams':
          ...
          class_name = self.nama_identifier_with_typeparams(nama_identifier_with_typeparams)
  ...
  def searchable(self, tree, filepath='imports_ts.txt'):
    """
    searchable
      searchable_kw
  ...
  def import_item(self, tree):
    ...
    for item in anak(tree):
      if data(item) == 'import_container':
        ...
        search_mode = True
        filepath = joiner(here(__file__), '..', 'import_ts.txt')
