...
try:
  ...
  pass
import traceback
from schnell.app.printutils import tryex, printex
...
from schnell.app.stringutils import tabify_content_tab, tabify_content_space
...
class Generator:
  ...
  def tabify_content(self, content):
    tabify = [('' if item.startswith('}') else self.tab()) + item for item in content.splitlines()]
    return '\n'.join(tabify)
      ...
      for li in list_items.children:        
        ...
        if ei.data == 'expression_item':
          ...
          if couldbe_literal.data == 'literal':
            ...
            print(f'literal: {hasil_literal} berjenis {type(hasil_literal)}')
  ...
  def literal(self, couldbe_literal):
    ...
    if nilailiteral.children:
      if nilailiteral.data == 'literal_string':
        ...
        print(f'kembalian_literal number: {kembalian_literal} berjenis {type(kembalian_literal)}')
  ...
  def enum_body(self, tree):
    ...
    for index, item in enumerate(anak(memberlist)):
      if data(item) == 'enum_member':
        hasil = self.enum_member(item, append_iota=True if index==0 else False)
  ...
  def tipe_identifier(self, tree):
    ...
    for item in anak(tree):
      ...
      elif jenis in ['array', 'dict', 'pair', 'set', 'tuple']: # ['dataframe', 'directory', 'file', 'network', 'orm']
        if jenis == 'array':
      ...
      if data(item) == 'penanda':
        ...
        for cucu in anak(item):
          ...
          if jenis == 'allocate':
            ...
            if jumlahanak(cucu) == 2 and chdata(cucu, 2) == 'tipe_data_nilai':
              # ada tipe data nilai
              tipe_identifier_untuk_nilai = child(child2(cucu))
              tipe_data_nilai = self.tipe_identifier(tipe_identifier_untuk_nilai)
              # print('oprek tipe_data_nilai', data(tipe_identifier_untuk_nilai), 'peroleh:', tipe_data_nilai)
            elif jumlahanak(cucu) == 3 and chdata(cucu, 3) == 'tipe_data_kunci':
              ...
              tipe_data_kunci = self.tipe_identifier(tipe_identifier_untuk_kunci)
            ...
            hasil = get_by_datatypes(tipe_data_nilai, int(jumlah), as_list=True)
    ...
    for item in anak(tree):
      ...
      if data(item) == 'penanda':
        ...
        if beranak(item) and chdata(item) == 'nama_identifier':
          iden = chtoken(item)
        self.inc()
        kembali += f'for index, {iden} in enumerate({identifier}):\n{self.tab()}pass'
        self.dec()
  ...
  def for_end(self, forend):
    ...
    print(f'forend, range oper: {oper}, end/kanan: {kanan}, type kanan: {type(kanan)}')
    if data(oper) == 'operator_less':
      return kanan
    elif data(oper) == 'operator_less_equal':
      return kanan-1
  ...
  def for_each(self, tree):
    ...
    for item in anak(tree):
      if data(item) == 'item_name':
        ...
        value = self.nama_jenis_identifier_optional(nama_jenis_identifier_optional)
      elif data(item) == 'array_name':
        items = chtoken(item)
      elif data(item) == 'key_name':
        nama_jenis_identifier_optional = child1(item)
        index = self.nama_jenis_identifier_optional(nama_jenis_identifier_optional)
  ...
  def method_content(self, tree):
    ...
    for item in anak(tree):
      if data(item) == 'function_name':
        ...
        methodname = self.nama_identifier_with_typeparams(nama_identifier_with_typeparams)
        # kembali += f'{nama}'
      elif data(item) == 'tipe_identifier':
    ...
    if self.track_methods_for_interface:
      ...
      kembali = f'func {methodname}{methodparam} {methodtype} {{ {methodcontent} }}'
    if self.track_methods_for_interface:
  ...
  def go_initialize_field(self, structname, fieldname, fieldtype, nilai):
    ...
    hasil = go_struct_template \
      .replace('__structname^', structname.capitalize()) \
  ...
  def field_content(self, tree, structname=None):
    ...
    for item in anak(tree):
      if data(item) == 'field_config':
        ...
        kembali_impl = self.go_initialize_field(structname, field_name, field_type, nilai)
  ...
  def classcontent(self, tree, structname=None):
    ...
    if ctor_method_field.data == 'constructor_content':
      ...
      item, field_impl = self.field_content(ctor_method_field, structname=structname)
      return item, field_impl
  ...
  def class_content(self, tree, structname=None):
    ...
    if beranak(classcontentlist):
      hasil, field_name_impl = self.classcontentlist(classcontentlist, structname=structname)
      kembali = hasil
  ...
  def search_term(self, terms, filepath):
    """
    """
    from schnell.app.greputils import pattern_search
    hasil = pattern_search(filepath, terms)
    return hasil


  def searchable(self, tree, filepath='imports_go.txt'):
    """
    searchable
      searchable_kw
  ...
  def import_item(self, tree):
    ...
    for item in anak(tree):
      ...
      #     merged_imports.append(item)
        ...
        search_mode = True
        filepath = joiner(here(__file__), '..', 'import_go.txt')
