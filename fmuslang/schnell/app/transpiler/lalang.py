...
# app.transpiler.lalang = os.pardir * 2
disini = os.path.normpath(os.path.abspath(os.path.dirname(__file__)))
path_app = os.path.join(disini, os.pardir)
...
from schnell.app.printutils import print_json, indah3
...
# )
# from lark.visitors import InlineTransformer
from schnell.app.transpiler.grammar import bahasa
from schnell.app.transpiler.config import program_context, daftar_languages
...
def myrepl():
	...
	while code != 'x':
		try:
			code = input(f'{datetime.datetime.now().strftime("%m/%d/%Y, %H:%M:%S")} >> ')
			code = code.strip()
			if code == 'bahasa':
				print(bahasa)
				trycopy(bahasa)
