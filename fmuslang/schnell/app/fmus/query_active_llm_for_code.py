...
def query_active_llm_for_code(oper, item, root_tree, self_run_configuration=None):
    print('query_active_llm_for_code')
    ...
    item.workdir = get_input_generic_if_input_not_from_template(
        item.workdir, item.parent, "query_active_llm_for_code"
    )
    content = get_input_generic_if_input_not_from_template(
        content, item.parent, "query_active_llm_for_code"
    )

    if env_int("ULIBPY_FMUS_DEBUG") > 1:
        indah4(f"[query_active_llm_for_code] content utk {item.workdir}: {content}")
    ...
    if root_tree is not None and hasattr(root_tree, "variables"):
        for k, v in root_tree.variables.items():
            ...
            if env_int("ULIBPY_FMUS_DEBUG") > 1:
                print(f"[query_active_llm_for_code] replacing {k} with {v} in {content}.")
    ...
    if env_int("ULIBPY_FMUS_DEBUG") > 1:
        # workdir adlh file
        indah4("[query_active_llm_for_code] END.")
