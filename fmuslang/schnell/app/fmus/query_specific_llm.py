...
def query_specific_llm(oper, item, root_tree, self_run_configuration=None):
    print('query_specific_llm')
    # dari processor utk jenis yg ada 2 variable
    # oper: query_specific_llm={'groq': 'create funny story about bubba'}
    ...
    item.workdir = get_input_generic_if_input_not_from_template(
        item.workdir, item.parent, "query_specific_llm"
    )
    content = get_input_generic_if_input_not_from_template(
        content, item.parent, "query_specific_llm"
    )

    if env_int("ULIBPY_FMUS_DEBUG") > 1:
        indah4(f"[query_specific_llm] content utk {item.workdir}: {content}")
    ...
    if root_tree is not None and hasattr(root_tree, "variables"):
        for k, v in root_tree.variables.items():
            ...
            if env_int("ULIBPY_FMUS_DEBUG") > 1:
                print(f"[query_specific_llm] replacing {k} with {v} in {content}.")
    ...
    new_content = invoke_llm(content, llm_name=provider, verbose=False)
