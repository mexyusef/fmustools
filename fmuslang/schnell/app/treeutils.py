...
from schnell.app.stringutils import tabify_contentlist
...
def replace_if_input_and_parent_is_dir(item):
	...
	if hasattr(item, 'parent') and item.parent:
		if item.parent.type == 'dir' and ayah(item.parent.workdir,1)==ayah(item.workdir,2):
			# extra checking
	...
	if ayah(item.workdir,1) .endswith(env_get('ULIBPY_FMUS_INPUT_KEYWORD')) and has_dir_parent:
		kembali = kembali.replace(env_get('ULIBPY_FMUS_INPUT_KEYWORD'), item.parent.name)
...
def replace_workdir__INPUT__with_value_from_parent(item):
	...
	parent:
		workdir='C:\\work\\tmp\\emih'
		         ...
		         sama           beda
	"""
	from schnell.app.dirutils import ayah, basename
	from schnell.app.utils import env_get
	parent = item.parent
	...
	if (skip_input_item==skip_input_parent) and (yang_mau_diganti_dari_item==input_keyword):
...
def get_child_token(tree, n=0):
	return chtoken(tree, n)


def tables_from_rootnode(RootNode):
	"""
	RootNode:
	AnyNode(dbname='hapuslah', dbtype='sqlite', host='localhost', label='root', output='', outputs=[], password='rahasia', port=5432, type='root', username='usef')
	table node:
	AnyNode(faker=5, model='User', name='table', type='table')
	"""
	node_tables = lambda node: hasattr(node, 'model') and node.name == 'table' and node.type == 'table'
...
def assignment_paramlist_type(TableNode, pemetaan=None, delimiter=', ', num_tab=0, tabber=tab_tab):
	from schnell.app.libpohon.handlers import type_mapper_by_provider
	result = []
...
def assignment_pydict_rest(TableNode):
	...
	# 	col_label = TableNode.children[0].label
	# 	return f'"{col_label}": {col_label}'
	paramlist = [f'"{col.label}": {col.label}' for i,col in enumerate(TableNode.children) if i>0]
	return ', '.join(paramlist)
